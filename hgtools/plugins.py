...
def patch_egg_info(force_hg_version=False):
    ...
    def tagged_version(self):
        vcs_param = getattr(self.distribution, 'use_vcs_version', False) or getattr(
            self.distribution, 'use_hg_version', False
        )
...
def _calculate_version(mgr, options):
    ...
    return (
        mgr.get_current_version(default_increment) if repo_exists else default_increment
    )
...
def calculate_version(options={}):
    ...
    has_tag_build = parser.has_section('egg_info') and 'tag_build' in parser.options(
        'egg_info'
    )
    if has_tag_build:
